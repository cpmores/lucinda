...
func (e *executor) execute(ctx context.Context, msg *APITaskmsg.TaskAssignMsg) error {
	...
	if e.sm != nil {
		...
		log.Println("executor: sm is nil!")
	}
	...
	// Publish result — the board handler cascades via Complete and
	// the reducer may need the task still in the tracer, so do NOT
	// remove it here. The board handler cleans up after Complete.
