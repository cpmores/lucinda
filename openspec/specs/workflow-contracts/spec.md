...
Defines the internal payloads and per-component API of the task workflow: raw task, `TaskPlan`, `Task`, `TaskSpec`, `TaskAd`, `TaskResult`; what each component (TaskWrapper, TaskPlanner, TaskCommander, TaskExecutor) consumes and produces, and where each result is routed.
