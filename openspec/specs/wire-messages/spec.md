...
Defines the cross-node message types in `api/v1/messaging/taskmsg` used for broadcast (task advertisement), assignment (task leased to an executor), and result (executor output routed back), carrying the `Owner` NodeID as the routing key.
