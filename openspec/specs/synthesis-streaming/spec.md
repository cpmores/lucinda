...
Defines how the final answer's token chunks are generated on the node that executes the synthesis task — possibly remote — and streamed over the dedicated transport protocol to the plan owner, independent of where the commander runs. A streamed answer ends with exactly one `done` SSE frame carrying the terminal plan status.
