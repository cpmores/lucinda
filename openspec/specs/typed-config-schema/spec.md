...
Defines the node configuration as a declarative, versioned, typed manifest: a `NodeConfig` manifest with `apiVersion`, `kind`, `metadata`, and `spec` that unmarshals into typed Go structs, is version-checked, and identifies the node for a mesh.
