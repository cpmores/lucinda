...
Defines how the final ReAct answer reaches the client: the commander streams it over the dedicated transport protocol when the loop reaches `done`, intermediate reasoning is never streamed, and the stream closes with a single `done` SSE frame.
